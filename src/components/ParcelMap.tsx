  ...
  return (
    <div className="relative isolate h-64 overflow-hidden border border-line">
      ...
      >
        ...
        {exact ? (
          <>
            <Circle center={[exact.lat, exact.lng]} radius={140} pathOptions={{ color: gold, weight: 1, fillColor: gold, fillOpacity: 0.16 }} />
            <CircleMarker center={[exact.lat, exact.lng]} radius={6} pathOptions={{ color: '#0e0e11', weight: 2, fillColor: gold, fillOpacity: 1 }} />
          </>
        ) : (
          area && <Circle center={[area.lat, area.lng]} radius={area.radiusKm * 1000} pathOptions={{ color: gold, weight: 1, fillColor: gold, fillOpacity: 0.07, dashArray: '4 4' }} />
        )}
      </MapContainer>
